...
- HW 02 due Thursday, September 24 at 11:59pm
...
::: question
Which components of the linear regression model $\mathbf{y} = \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\epsilon}$ are fixed, i.e., not random variables?
:::

![](images/september-17-qrcode.png){fig-align="center" width="35%"}

<center><https://app.wooclap.com/SFPYPFZ></center>
...
Let $\mathbf{z}$ a $p \times 1$ vector of random variables. Then
...
**Formula for** $\mathbf{Az}$

Let $\mathbf{A}$ be an $n \times p$ matrix of constants

\begin{aligned}
Var(\mathbf{Az}) &= E[(\mathbf{Az} - E(\mathbf{Az}))(\mathbf{Az} - E(\mathbf{Az}))^T]\\& = \mathbf{A}Var(\mathbf{z})\mathbf{A}^T
\end{aligned}
...
- (Optional) submit team request by end of the day

- Complete [Lecture 09 prepare](../prepare/prepare-lec09.html): Distribution of coefficients
